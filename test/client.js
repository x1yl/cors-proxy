document.addEventListener('DOMContentLoaded', () => {
	...
	async function testXCorsHeaders() {
		...
		const foundSpecialHeader = Object.keys(specialHeaders).some((key) => data.receivedHeaders[key.toLowerCase()] === specialHeaders[key]);
...
=== TEST RESULTS BY CATEGORY ===\n\n`;
		...
		for (const [category, results] of Object.entries(categorizedResults)) {
			...
			const passRate = categorySummary.total > 0 ? ((categorySummary.passed / categorySummary.total) * 100).toFixed(1) + '%' : 'N/A';

			reportText += `\n${category} (Pass Rate: ${passRate})\n${'-'.repeat(category.length + 20)}\n`;

			results.forEach((result, index) => {
				const statusText = result.status === 'passed' ? 'PASS' : result.status === 'failed' ? 'FAIL' : 'RUNNING';

				reportText += `${index + 1}. ${result.option}: [${statusText}]\n`;
				if (result.details && result.status !== 'running') {
					reportText += `   ${typeof result.details === 'string' ? result.details : JSON.stringify(result.details, null, 2)}\n`;
				}
			});
